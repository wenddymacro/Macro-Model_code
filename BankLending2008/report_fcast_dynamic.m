...
function report_fcast_dynamic(fy, F, Udecomp, ndiffuse, freq, nahead, exo_ord, ...
                              string_date)
...
for ib=1:nb
  ...
  draw_dynamic_graph(fy, F, Udecomp, nahead, exo_ord, ffy, ffq, (ib-1)*batch_len+ndiffuse, len, ib);
end
...
function draw_dynamic_graph(fy, F, Udecomp, nahead, exo_ord, first_year, ...
                            first_quarter, offset, len, id)
global options_ M_ lgx_
...
labs = remove_prefix(lgx_);
cml = size(cmap,1);
ind = floor(1:cml/(size(lgx_,1)+1):cml);
cm = cmap(ind,:);
...
for ivar=1:nvar
  ...
  subplot(3,1,3);
  [uall llabels sel] = select_large_u(uall, exo_ord, labs, 6);
  ipos = find(uall>=0);
