% [fy, fx, Fyall, Fxall, Udecomp, ndiffuse] = calc_fcast_all(nahead)
%
...
function [fy, fx, Fyall, Fxall, Udecomp, ndiffuse] = calc_fcast_all(nahead)
...
Fyall = zeros(nobs, nahead, gend+1);
Fxall = zeros(size(dr_.ghx,1), nahead, gend+1);
...
while t < smpl
  ...
  for h=1:nahead
    ...
    af = T*af;
    Fyall(:,h,t) = diag(Pstar1(mf,mf));
    Fxall(:,h,t) = diag(Pstar1(1:size(Fxall,1),1:size(Fxall,1)));
