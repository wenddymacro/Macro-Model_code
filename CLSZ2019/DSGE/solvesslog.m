function [k, i, uc, c, h ,w,r ,pi, uk, rk, x, y, wes,wep, ys, ks, hes, hhs, ns, bs, as ,rs, omegas, yp, kp, hep, hhp, np, bp, ap, rp, omegap, tau,gdp,ytfp,gtfp,rzs,rzp,g,bcs,bcp,ps,pp] = solvesslog(DELTA, G, OMEGAK, PSI, ETA, BETA, EPS, OMEGAP, PISS, ALPHA, THETA, MS,MP, XIS,XIP, K, OMEGAM, AS,AP, LS, LP, TAU,GSS,SIGMAM,PSIY)
...
gdp = c+i+g;
ytfp = y/((ks+kp)^(1-ALPHA)*((h)^(THETA))^ALPHA);


gtfp = gdp/((ks+kp)^(1-ALPHA)*(h^(THETA))^ALPHA);% Eq. 43
utility = (log(c)-PSI*h^(1+ETA)/(1+ETA));
rzs  = omegas*as*(ns/PISS/G+bs)/bs;% Eq. 23
rzp  = omegap*ap*(np/PISS/G+bp)/bp;% Eq. 23
bcp = 1-(OMEGAM/omegap)^K;% Transform bankruptcy cutoff to bankruptcy ratio
bcs = 1-(OMEGAM/omegas)^K;
